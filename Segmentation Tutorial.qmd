...
Typically, I would run various checks to identify the optimal cluster number, but for the purposes of comparing methods, I will stick to a 4 cluster solution.
...
dat_num$kclusters = k_means$cluster

# Pivot long to plot proportions
plot_data <- dat_num %>%
  pivot_longer(
...
ggplot(plot_summary, aes(x = Variable, y = MeanScore,
                         color = factor(kclusters), group = kclusters)) +
  geom_line(size = 1.1) +
  geom_point(size = 2) +
  facet_wrap(~ kclusters, scales = "free_x") +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 10)
  ) +
  labs(y = "Proportion Selected", x = "Genre", color = "Cluster")
```
...
```{r}
set.seed(10)
k_means <- MiniBatchKmeans(corrmat, clusters = 4, batch_size = 500, num_init = 10)
clusters = predict_KMeans(corrmat, k_means$centroids)
...
```{r}
dat_num_nozerovar$corrclusters = clusters
...
ggplot(plot_summary, aes(x = Variable, y = MeanScore,
                         color = factor(corrclusters), group = corrclusters)) +
  geom_line(size = 1.1) +
  geom_point(size = 2) +
  facet_wrap(~ corrclusters, scales = "free_x") +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 10)
  ) +
...
J <- matrix(0, nrow(dat_num), nrow(dat_num))
nz <- union > 0
...
```{r}
set.seed(10)
k_means <- MiniBatchKmeans(jaccard_sim, clusters = 4, batch_size = 500, num_init = 10)
clusters = predict_KMeans(jaccard_sim, k_means$centroids)
...
ggplot(plot_summary, aes(x = Variable, y = MeanScore,
                         color = factor(jaccclusters), group = jaccclusters)) +
  geom_line(size = 1.1) +
  geom_point(size = 2) +
  facet_wrap(~ jaccclusters, scales = "free_x") +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, color = "black", size = 10)
  ) +
