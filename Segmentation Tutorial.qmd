...
The following polar figure plots the proportions of genres selected by each cluster. I chose a circular format because the shape quickly visually shows whether there are magnitude differences (larger vs smaller more circular shapes) or specific genre tastes (directional shapes that point towards constellations of genres). It corroborates the cluster interpretations above from the biplot. The decimals after the cluster name reflect the proportion of the sample in each cluster.

```{r}
# Collect proportion of sample for each cluster
cluster_props <- dat_num %>%
  mutate(kclusters = k_clusters) %>%
  count(kclusters) %>%
  mutate(prop = weights::rd(n / sum(n), 2)) %>%
  mutate(label = paste0("Cluster ", kclusters, ": ", prop))

# Pivot long to plot proportions by cluster
plot_data <- dat_num %>%
  # Add cluster labels
  mutate(kclusters = k_clusters)  %>%
  left_join(cluster_props[, c("kclusters", "label")], by = "kclusters") %>%
  mutate(kclusters = factor(label)) %>%
  pivot_longer(
...
```{r}
# Collect proportion of sample for each cluster
cluster_props <- dat_num_nozerovar %>%
  mutate(corrclusters = corr_clusters) %>%
  count(corrclusters) %>%
  mutate(prop = weights::rd(n / sum(n), 2)) %>%
  mutate(label = paste0("Cluster ", corrclusters, ": ", prop))

plot_data <- dat_num_nozerovar %>%
  mutate(corrclusters = corr_clusters) %>%
  left_join(cluster_props[, c("corrclusters", "label")], by = "corrclusters") %>%
  mutate(corrclusters = factor(label)) %>%
  pivot_longer(
    cols = c(AltRock:Soul),  # all clustering variables
    names_to = "Variable",
    values_to = "Score"
...
cluster_props <- dat_num %>%
  ...
  count(jaccclusters) %>%
  mutate(prop = weights::rd(n / sum(n), 2)) %>%
  mutate(label = paste0("Cluster ", jaccclusters, ": ", prop))

plot_data <- dat_num %>%
  mutate(jaccclusters = jacc_clusters) %>%
  left_join(cluster_props[, c("jaccclusters", "label")], by = "jaccclusters") %>%
  mutate(jaccclusters = factor(label)) %>%
  pivot_longer(
    cols = c(AltRock:Soul),  # all clustering variables
    names_to = "Variable",
    values_to = "Score"
...
# Plot the biplot
scores <- as.data.frame(pca$x[, 1:2])    # respondent positions
scores$cluster <- factor(lca_classes)    # add cluster assignment
...
classprop = data.frame(Class = paste0('class ', 1:(k), ": "), 
                      Proportion = weights::rd(modlist[[k-1]]$P, 2)) %>% 
...
modlist[[k-1]]$probs %>% 
    ...
    left_join(classprop) %>%
    mutate(Class = paste0(str_to_title(Class), Proportion)) %>%
